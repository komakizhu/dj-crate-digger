...
平台字段先按自然语言确定模式，再执行搜索：

1. 写出“唯一平台 / 仅限 / 只使用 / 只在 / 只要 / 仅接受”等独占表达时，设置 `exclusive`，`allowed` 只保留明确的平台；目标平台没有直接页时记录“目标平台缺失”，不得用其他平台回退。
2. 如果用户在该字段只填写一个平台，且没有“优先 / 最好有 / 备用 / 可回退”等词，默认同样设置 `exclusive`。例如只填写“网易云”时，不把其他平台自动加入 `allowed`，也不把默认 SoundCloud、Apple Music、Spotify 顺序当成隐含回退。
3. 填写多个平台但没有独占表达时，设置 `preferred`，`allowed` 只能包含用户列出的平台，`priority` / `link_priority` 按填写顺序排列；这些列出的平台可以按顺序回退，不把其他平台加入 allowed。用户明确说“优先 / 最好有 / 备用 / 可回退”时，再把首选与回退关系按原话记录。
4. 没有填写平台时才使用 `cross-platform` 和默认平台顺序。用户写“不要 X / 不提供 X”时，X 是 `forbidden`，不得搜索、引用或输出。

“优先/最好有”只改变允许平台之间的链接顺序和回退顺序；它不改变内容排序权重。候选记录的 `primary_source` 应是第一顺序中已验证的允许平台页；Bandcamp、Beatport 等权威页放入补充证据，只有没有任何允许平台直接页时才可作为非 exclusive 报告的主证据。
