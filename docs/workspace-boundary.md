...
老炮 DJ 只负责生成推荐报告和用户明确请求的 UTF-8 `.w4dj` 交接文件；它不下载音频、不读取 W4DJ 数据库、不处理本地音频，也不负责导出 Rekordbox 播放列表。跨项目联动只经过 `.w4dj` 文件：歌单名、曲目顺序、完整官方歌名、艺人，以及固定为 JSON `null` 的 `netease_track_id` 兼容字段。DJ Skill 不搜索、推断或验证歌曲级网易云 ID；W4DJ 在下游记录用户实际选中的身份和输出文件绑定。
