...
完整推荐报告输出后，无论用户是否请求平台歌单，都要进入统一的“下一步”收尾，并先把 `local_export.status` 与 `w4dj_export.status` 设为 `offered`。这个收尾只允许出现在完整推荐报告之后，不得在两轮需求收集期间提前出现，也不是第三轮需求收集。简要版、丰富版和极速版都显示自然语言反馈邀请以及 W4DJ / 文字版歌单导出句；简要版和丰富版另外显示五度圈排序邀请，极速版不显示五度圈邀请。默认不展示私人记忆状态、本轮反馈状态、长期画像状态、缺失信息或来源栏目；这些状态仍在后台记录。
