...
第二，您可以输出文字版歌单或交接给 W4DJ，直接回复`输出文字版歌单`或者`导出到w4dj`即可
~~~
...
- [ ] 在 export.md 固定 `输出文字版歌单` / `导出到w4dj` 指令，同时保持导出状态机、UTF-8、顺序、去重、直接链接和不覆盖旧文件规则不变；W4DJ 使用全新的 v2 最小结构，旧 v1 直接拒绝且不迁移。
- [ ] 运行检查：

~~~bash
rg -n '下一步|选歌碎碎念|导出txt|导出到w4dj|输出文字版歌单|不是第三轮需求收集|不算第三轮需求收集' SKILL.md references/report-template.md references/export.md
~~~
