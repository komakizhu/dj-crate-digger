...
综合版是所有已触发 DJ 请求的默认输出；只有用户明确指定极速版或完整版时才切换。信息明确或用户说“其余你判断、直接出结果”时，Skill 跳过问卷直接执行。信息已经表达出至少两个关键方向、但还缺少时长等信息时，先只问一次“直接出结果，还是走两轮问卷细化”；选择直接出结果后由 Skill 判断低影响细节，选择问卷后才进入两轮固定问卷。只有几乎没提供有效信息的请求才直接进入第一轮。用户即使已经看到第一轮，之后说“直接出结果”也可以退出剩余问卷。问卷第一轮收集场景、目标国家/地区、核心声音方向、歌曲数量或 Set 时长、输出版本和其他限制；第二轮收集具体风格、BPM、熟悉度与发现感、时代与经典、情绪、能量走势、平台与链接要求和其他。两轮中没有明确选其他模式，最后仍默认输出综合版。问卷均来自固定语言包，不增加第三轮普通需求收集。
...
| 模式 | 适合场景 | 输出 |
|---|---|---|
| 综合版（默认） | 获取一份完整歌单 | 一张严格十二列表格、调性、选择理由和一条创造性接歌建议 |
| 完整版 | 比较不同选歌方向 | 风格、场景、熟悉度/发现感三个专项视角，加一个动态综合视角 |
| 极速版 | 先快速拿到可用曲目 | 独立三列表、单一召回通道、分段首批与续跑规则 |

综合版和完整版的表格固定为：`歌名 | 艺术家 | 专辑/EP | 风格 | BPM | 调性 | 歌曲时长 | 能量级 | 发行时间 | 简介 | 选择原因 | 链接`。所有模式都保留完整官方歌名、平台策略、逐曲证据和录音级去重。为兼容旧请求，`简要版`仍按综合版理解，`丰富版`仍按完整版理解，但新提示和报告只使用新名称。

最终输出模板也分为固定区和动态区：模式/视角标题、表头及顺序、`下一步`的数量/顺序/标题/正文、命令词、教程链接和 Markdown 格式必须原样保留，不得改写、压缩、合并、调序或删除。综合版和完整版的三项“下一步”中，第二项必须完整保留文字歌单与 W4DJ；极速版的两项也必须完整保留。歌曲数据、歌单名称、选歌碎碎念正文和接歌建议正文才是可动态生成的部分。
...
调性只接受与同一完整官方歌名对应的可靠证据；未知或冲突时保留歌曲并显示未知。综合版和完整版收到明确肯定后，才按基础 Camelot 兼容关系进行五度圈重排，并提供最多五组需要 DJ 试听确认的 double drop 候选。没有波形或实际试听证据时，接歌建议只用“可尝试”的创造性表达。
